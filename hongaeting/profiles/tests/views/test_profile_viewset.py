...
class ProfileTestCase(APITestCase):
    def test_should_get_profile_list(self):
        ...
        profile = baker.make('profiles.Profile', _quantity=profile_quantity)
        ...
        for i in range(profile_quantity):
            assert_that(response.data[i]['nickname']).is_equal_to(profile[i].nickname)
            assert_that(response.data[i]['gender']).is_equal_to(profile[i].gender)
            assert_that(response.data[i]['age']).is_equal_to(profile[i].age)
            assert_that(response.data[i]['height']).is_equal_to(profile[i].height)
            assert_that(response.data[i]['body_type']).is_equal_to(profile[i].body_type)
            assert_that(response.data[i]['tag']).is_equal_to(profile[i].tag)
            assert_that(response.data[i]['image']).is_equal_to(profile[i].image)
            assert_that(response.data[i]['appearance']).is_equal_to(profile[i].appearance)
            assert_that(response.data[i]['personality']).is_equal_to(profile[i].personality)
            assert_that(response.data[i]['hobby']).is_equal_to(profile[i].hobby)
            assert_that(response.data[i]['ideal_type']).is_equal_to(profile[i].ideal_type)
            assert_that(response.data[i]['last_tempting_word']).is_equal_to(profile[i].last_tempting_word)
